...
trait SocketGlobal
{
    ...
    protected function server(){
        $address=isset($this->config['global']) ? explode(':',$this->config['global']) : ['0.0.0.0','22075'];
        $this->server=new Server($address[0],$address[1]);
        return $this;
    }

    protected function client(){
        $address=isset($this->config['global']) ? $this->config['global'] : '0.0.0.0:22075';
    ...
    protected function addSubUpdate($type='public',$data=[]){
        do{
            ...
            foreach ($new_value as $k=>$v){
                if($type=='public' && !array_key_exists('key',$v)) unset($new_value[$k]);
                if($type=='private') {
                    //添加的频道必须当前用户
                    $key=$v['key'];
                    if(array_key_exists('key',$v) && $key==$data['user_key']) unset($new_value[$k]);
                }
            }
        }
    ...
    protected function allSubUpdate($type='public',$data=[],$keysecret=''){
        do{
            $old_value=$new_value=$this->client->all_sub;
            foreach ($data['sub'] as $v){
                if($type=='public') $key=json_encode($v);
                if($type=='private') {
                    $key=$keysecret['key'];
                    $key.=self::$USER_DELIMITER.json_encode($v);
                }
                $new_value[$key]=$v;
            }
        }
    ...
    protected function unAllSubUpdate($type='public',$data=[],$keysecret=''){
        do{
            $old_value=$new_value=$this->client->all_sub;
            foreach ($data['sub'] as $v){
                if($type=='public') unset($new_value[json_encode($v)]);
                if($type=='private') {
                    $key=$keysecret['key'];
                    $key.=self::$USER_DELIMITER.json_encode($v);
                    echo $key.PHP_EOL;
